...
namespace brechtbaekelandt.tools.Helpers.JsonClassGenerator.CodeWriters
{
    public class TypeScriptCodeWriter : ICodeWriter
    {
        ...
        public string GetTypeName(JsonType type, IJsonClassGeneratorConfig config)
        {
            switch (type.Type)
            {
                ...
                case JsonTypeEnum.Array: return this.GetTypeName(type.InternalType, config) + "[]";
                case JsonTypeEnum.Dictionary: return "{ [key: string]: " + this.GetTypeName(type.InternalType, config) + "; }";
        ...
        {
            var prefix = this.GetNamespace(config, type.IsRoot) != null ? "    " : "";
            ...
            foreach (var field in type.Fields)
            {
                ...
                sw.WriteLine(prefix + "    " + field.JsonMemberName + (this.IsNullable(field.Type.Type) ? "?" : "") + ": " + (shouldDefineNamespace ? config.SecondaryNamespace + "." : string.Empty) + this.GetTypeName(field.Type, config) + ";");
            }
            sw.WriteLine(prefix + "}");
            sw.WriteLine();
        }
        ...
        {
            if (this.GetNamespace(config, root) == null)
            {
                return;
            }
            sw.WriteLine("module " + this.GetNamespace(config, root) + " {");
            sw.WriteLine();
        }
